...
/*
	...
	ajaxcheck.php - Called when creating new job - may also call ajaxcheckdep.php if client has departments
...
if(isSet($_POST['newjobselectclient'])) {
    $newjobclientid = $_POST['newjobselectclient'];
    include "C4uconnect.php";
    
    $query="SELECT * FROM Clients WHERE Clients.CustomerID = ? LIMIT 1"; 
    ...
    if ($row) {
        ...
        // reduce to 1 invoice lookup loop
        ...
        $today = date('Y-m-d H:i:s', $dateOneMonthAdded);
        $sql = "SELECT SUM(cost + invvatcost) AS cost FROM invoicing  
        WHERE (`invoicing`.`paydate` =0 ) 
        AND (`invoicing`.`invdue` < '$today' ) 
        ...
        if ($num_rows>'0') {
            while ($irow = mysql_fetch_array($sql_result)) {
                extract($irow); 
                $tablecost+=$irow['cost'];
            } 
            ...
            $itablecost='0'; 
            $isql = "SELECT SUM(cost + invvatcost) AS cost , invdue FROM invoicing  
            WHERE (`invoicing`.`paydate` =0 )
            AND (`invoicing`.`client` = '$newjobclientid' ) ";
            //   echo $sql;
            $isql_result = mysql_query($isql,$conn_id) or die(mysql_error()); 
            $inum_rows = mysql_num_rows($isql_result);
            $inum_rows=$inum_rows-$num_rows;
            if ($inum_rows>'0') {
                while ($irow = mysql_fetch_array($isql_result)) {
                    extract($irow);
        ...
        if ($row['isdepartments']=='1') {
            ...
            while ($favrow = mysql_fetch_array($sql_result)) {
                extract($favrow);
                $html.= '
                <option value="'.$favrow['favadrid'].'"';
                if ($favrow['favadrid']==$row['defaultfromtext']) { $html.= ' SELECTED '; }
        ...
        $html.= ' <button name="showallfav" title="All Favourites" id="showallfav" type="button" class="showallfav"> &nbsp; </button> ';
        ...
        $html.= ("<select class=\"jlabel ui-state-default ui-corner-left\" name=\"serviceID\">"); 
        ...
        $html.= '<select class="ui-state-default ui-corner-left" name="ajcolldue">';
        ...
        $html.= '<select class="jlabel ui-state-default ui-corner-left" name="ajdelldue">';
        $html.= ' <option value="now">Now </option>';
        ...
        $html.= '<div class="fs" > <div class="fsl"> </div>
