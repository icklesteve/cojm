...
$rhtml.= 'PU '.$fromfreeaddress.' <a target="_blank" class="newwin" href="http://maps.google.com/maps?q='. $linkCollectPC. '">'. $row['CollectPC'].'</a> ';

}

if (((trim($fromfreeaddress)) or (trim($row['CollectPC']))) and ((trim($tofreeaddress)) or (trim($row['ShipPC'])))) { $rhtml=$rhtml. '<br />'; }

if ((trim($tofreeaddress)) or (trim($row['ShipPC']))) { $rhtml.= 'To '; 

$linkShipPC = strtoupper(str_replace(' ','+',$row['ShipPC'])); 	

$rhtml.= ''.$tofreeaddress.' <a target="_blank" class="newwin" href="http://maps.google.com/maps?q='. $linkShipPC.'">'. $row['ShipPC'].'</a>';
...
$areaquery = "SELECT opsmapid, opsname, descrip, istoplayer FROM opsmap WHERE opsmapid='$opsmaparea' "; 
 ...
 if ($row['opsmapsubarea']) { $rhtml.=' ( Sub Area ) '. $row['opsmapsubarea'];  }
...
if ($row['orderdep']) {
$orderdep=$row['orderdep']; $depquery="SELECT depname FROM clientdep WHERE depnumber = '$orderdep' LIMIT 1";
...
$rhtml.='
<a href="cyclist.php?thiscyclist='.$row['CyclistID'].'">'.$row['cojmname'].'</a>';
}
...
// $rhtml=$rhtml. '<tr><td> </td><td> </td><td> </td><td> </td><td> </td><td> </td><td> </td></tr>';
 ...
 $html= '
<table style="width:100%;" class="acc"><tbody><tr>
...
$html=$html.'</tbody></table>'.$podfound.'';

$html=$html. '<div class="vpad line"></div>';
}
...
echo '<div class="ui-widget">
			<div class="ui-state-highlight ui-corner-all p15" > 
				...
				<strong>'.$fwrtot.' Job';
				if ($fwrtot<>1) { echo 's'; }
				echo ' awaiting admin </strong> &'. $globalprefrow['currencysymbol'].$fwrcost.' inc vat.<br />
				<strong> '.$numit.' Jobs awaiting invoicing </strong> &'. $globalprefrow['currencysymbol'] . $tablecost.' inc vat<br />';
				
	 $lasttracked="
SELECT timestamp FROM `instamapper` ORDER BY `instamapper`.`timestamp` DESC LIMIT 0 , 1 "; 
...
$sql_result = mysql_query($sql,$conn_id)
 ...
 if ($sumtot>0)  { echo '<h4>'. $sumtot.' Jobs specifically awaiting Invoicing</h4>
<table cellspacing="0" class="acc"><tbody>
<tr>
...
echo ((date("Y")-$year)).' on '.date('l', strtotime(date("Y-$month-$day"))).', '.$day.' '.$month.' '.date("Y").'.</h4><br>';
 ...
 if ($todaycheck==date("m-d")) { echo "<h3>".$row['cojmname']."'s Birthday TODAY! (". ((date("Y")-$year)). ") </h3>";
...
if ($lastid<>'')	{ $rhtml=$rhtml. ' Last one is <a href="order.php?id='. $lastid.'">'. $lastid.'</a>'; }
...
if ($row['auditfilename'] == 'No Cron Ran' ) { $cronissue++;
...
if ($crondiff>5) { echo ' Oldest >5mins ago, resetting cron.'; 
